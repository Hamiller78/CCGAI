...
std::shared_ptr<GameState> GamemoveMoveTop::ApplyOnGamestate(const std::shared_ptr<GameState> oldState)
{
    if (nullptr == resultingState_)
    {
        std::shared_ptr<GameState> resultStatePtr(new GameState(*oldState));
        resultStatePtr->MoveTopPiece(startPosition_, targetPosition_);
